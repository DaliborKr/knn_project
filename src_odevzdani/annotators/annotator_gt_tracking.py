...
#Example usage: python src/annotators/annotator_gt_tracking.py -v splitted/test1/d2_01_120-159.mp4  -o ground_truth_tracks/gt_test1.json -int 0 40
...
parser.add_argument("-o", "--output", help="Path to output ground truth track file", default="gt_track.json")
parser.add_argument("-i", "--input", help="Path to input track file", default="def.json")
parser.add_argument("-int", "--interval", nargs=2, type=int, metavar=("START", "END"), help="Interval of chosen frames", default=[0, 1000])
...
def parseArgs():
    global inputVideoPath, inputTrackPath, outputTrackPath, firstFrame, lastFrame, current_frame

    args = parser.parse_args()

    inputVideoPath = args.video
    outputTrackPath = args.output
    inputTrackPath = args.input
...
try:
    with open(inputTrackPath, "r") as f:
...
while cap.isOpened():
    ...
    if key == ord("x"):  # Quit
        ...
        with open(outputTrackPath, "w") as f:
